...
# --- 1. Configuration ---
DATA_FILE = "frd_sample_futures_NQ/NQ_15min_sample.csv"
OUTPUT_DIR = "plots_swing_failures_15min"
RESULTS_BASE_DIR = "analysis_results_15min"
...
def load_data(filepath):
    ...
    # Calculate features for 15-minute data
    df['hour'] = df.index.hour # Still useful for hourly grouping of 15min patterns
    df['c0_height'] = abs(df['open'] - df['close'])

    # Calculate net change over previous periods (assuming 96 periods per day for 15min data)
    # We can add historical daily change like in the hourly script later if needed for 15min.
    # For now, focusing on core SFP analysis for 15min timeframe.
    # df['prev_1d_change'] = df['close'].pct_change(periods=96) * 100 # 24 hours * 4 (15min in an hour)
    # df['prev_2d_change'] = df['close'].pct_change(periods=192) * 100 # 48 hours * 4
    # df['prev_3d_change'] = df['close'].pct_change(periods=288) * 100 # 72 hours * 4
    
    # df.dropna(inplace=True) # Drop rows with NaN values resulting from pct_change if calculated

    return df

# --- Helper function to categorize daily change (from hourly, not directly used in 15min SFP analysis yet) ---
...
def analyze_swing_failures(df, c0_c1_relative_height_threshold, c1_retracement_depth_threshold):
    ...
    for i in range(len(df) - 2):
        ...
        # Previous day's category is not directly applicable to 15min patterns in the same way
        # We will not include prev_1d_cat in the 15min results for now.
...
def aggregate_and_print_stats(results_df, output_file=None):
    ...
    stats_summary = []
    # Aggregate by hour
    # Previous day's category aggregation is removed for 15min analysis for now
    ...
    for hour in range(24):
        ...
        # For 15min data, total candles per hour will be different
        ...
        # Create the hour triplet string - Note: this is still based on C0 hour, not a 15min interval triplet
        h0 = hour
        h1 = (hour + 1) % 24
        h2 = (hour + 2) % 24
        ...
        if bear_occurrences > 0 or bull_occurrences > 0:
            stats_summary.append({
                # 'Prev Day': category, # Removed for 15min analysis
    ...
    print("Aggregated 3-candle swing-failure stats for 15-minute data by Hour (NY time of C0):")
...
def plot_pattern_example(row, output_dir, filename_prefix="pattern"):
    ...
    candles_to_plot = []
    # Need to adjust time spacing for 15min data
    ...
    fig.add_trace(go.Candlestick(
        # Adjusted x-axis to space candles by 15 minutes
    ...
    fig.update_layout(
        title=f"{row['type'].capitalize()} Swing Failure Example (15min) - {row['timestamp']}",
        xaxis_title="Time", yaxis_title="Price",
        xaxis_rangeslider_visible=False
    )
    ts_str = row['timestamp'].strftime('%Y-%m-%d_%H%M') # Use minute in filename for 15min data
...
if __name__ == "__main__":
    ...
    run_dir_name = f"15min_height{current_c0_c1_height_threshold}_depth{current_c1_retracement_depth_threshold}_{timestamp}"
    ...
    # Pass original df to aggregate stats for total candle count per hour/category if needed later
    # aggregate_and_print_stats(pattern_results, swing_failure_results_filepath, df)
    ...
    pattern_results_filepath = os.path.join(current_results_dir, "raw_pattern_results_15min.csv")
    ...
    if not pattern_results.empty:
        ...
        for idx, row in bearish_examples.iterrows():
            plot_pattern_example(row, plots_output_dir, "bearish_sfp_15min")
        for idx, row in bullish_examples.iterrows():
            plot_pattern_example(row, plots_output_dir, "bullish_sfp_15min")
